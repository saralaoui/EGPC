/*****************************************************************************
 ...
 Sarah FDILI ALAOUI, Universit� Paris 11
...
//Pour la liaison OSC, d�finitio du socket et du buffer
...
//Param�tres � gerer par le biais d'un controle gestuel
...
// param�tres Cg pour le programme de calcul des forces
// pass 1: forces
...
// pass 4: positions
// param�tres Cg pour le programme de calcul des positions
...
// pass 5: dessin
// param�tres Cg pour le programme de dessin
...
GLvoid initGL()  {
	...
	// D�finit un mod�le d�ombrage
	glShadeModel(GL_SMOOTH);
	
	// Z Buffer pour la suppression des parties cach�es
...
GLvoid initCg()  {
	...
	// cr�ation d'un contexte pour les programmes Cg
	...
	if( cgQuadVertexProgram == NULL ) {
		...
		// dont on affiche la chaine - g�n�ralement peu informative...
...
// fonction de call-back de glut pour la gestion des �v�nements clavier
...
// fonction de call-back de glut de gestion des d�placement de la souris
...
// pour une texture RECT (coordonn�es de texture entre 
// 0 et width) dans le FS
// et via la normale pour une texture 2D (coordonn�es 
...
void MakeParticles( void ) {
	...
	for( int ind = 0 ; ind < NB_MASSES-1 ; ind++ ) {
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0 , 0.0);
		glVertex4f(  (float)(ind) , 3.0 , -0.01, (float)((ind+1) % (NB_MASSES-1)) );
		
		glTexCoord2f(128.0 , 0.0);
		glVertex4f(  (float)(ind) , 1.0 , -0.01, (float)((ind+1) % (NB_MASSES-1)) ); 
		
		glTexCoord2f(128.0 , 128.0);
		glVertex4f(  (float)((ind+1) % (NB_MASSES-1)) , 1.0 , -0.01,  (float)(ind) ); 
		
		glTexCoord2f(0.0 , 128.0);
		glVertex4f(  (float)((ind+1) % (NB_MASSES-1)) , 3.0 , -0.01, (float)(ind)); 
		glEnd();
	}
...
void MakePoints( void ) {
	// 1 point correspond � une masse au bout d'une ar�te
	// il est rendu � la position dans la texture de force
	// (donc aux coordonn�es de texture d'une masse dans 
	// une texture de donn�es de masses)
	...
	// les masses sont positionn�es au bon endroit du frame buffer
...
void CreateTexture( int textureId , int sizeX , int sizeY ) {
	...
	switch( textureId ) {
		case POSITION_1_TEXTURE:
		{
			...
			// utilis�e en shader de vertex et de fragment
			...
			dataFloatMasses[ 16 ] = 0;
			dataFloatMasses[ 17 ] = 0;
			dataFloatMasses[ 18 ] = 10;
			...
			for( int indTex = POSITION_1_TEXTURE ; indTex <= POSITION_3_TEXTURE ; 
				...
				// cibles d'�criture pour le FBO: la position pr�c�dente, 
				// la position courante et la position ant�rieure. 
				// On a besoin de ces trois valeurs pour
				// calculer le d�placement dans le sch�ma d'int�gration de Verlet
				// VTF (vertex texture fetch) donc format GL_RGBA_FLOAT32_ATI
				// attach�e � un FBO 
				// initialis�e par les valeurs stock�es dans dataFloat
				glTexImage2D( GL_TEXTURE_2D , 0, GL_RGBA_FLOAT32_ATI , 
							 sizeX, sizeY, 
							 // dataFloat est NULL pour POSITION_3_TEXTURE
							 // qui n'a pas besoin d'etre initialis�e
		...
		case FORCE_TEXTURE:
		{
			...
			// attach�e � un FBO et sans initialisation n�cessaire
		...
		case FORCE_MASS_TEXTURE:
		{
			...
			// la pr�cision doit �tre 16f (le blending sur les textures 
		...
		case EDGE_TEXTURE:
		{
			...
			glEnable( GL_TEXTURE_2D );
			// Lie la texture avec une cible carr�e
			...
			//les 4 premi�re masses sont en carr�
				
				for( int i = 0 ; i < NB_MASSES-1 ; i++ ) {
					dataFloatEdge[ i * 4 ]     = (GLfloat)(i % (NB_MASSES-1));
					dataFloatEdge[ i * 4 + 1 ] = (GLfloat)((i + 1) % (NB_MASSES-1));
					dataFloatEdge[ i * 4 + 2 ] = 0;
					dataFloatEdge[ i * 4 + 3 ] = 2;
				}
			for( int i = NB_MASSES-1 ; i < NB_EDGES ; i++ ) {
				dataFloatEdge[ i * 4 ]     = (GLfloat)(i % (NB_MASSES-1));
				dataFloatEdge[ i * 4 + 1 ] = 4;
				dataFloatEdge[ i * 4 + 2 ] = 0;
			...
			/*	for( int i = 0 ; i < NB_EDGES ; i++ ) {
					dataFloatEdge[ i * 4 ]     = (GLfloat)(i % NB_MASSES);
					dataFloatEdge[ i * 4 + 1 ] = (GLfloat)((i + 1) % NB_MASSES);
					dataFloatEdge[ i * 4 + 2 ] = (((float)rand() / (float)RAND_MAX) + 1) * 0.3;
					dataFloatEdge[ i * 4 + 3 ] = (((float)rand() / (float)RAND_MAX) + 1) * 2 * M_PI / (float)NB_MASSES;
				}*/
...
void render_scene() {
	...
	// et celle des positions ant�rieures
	...
	//   /////////////////////////////////////////////////////////
	// repassage de la texture de raideur et �longation pour modification des caract�ristiques des ressorts
	//   /////////////////////////////////////////////////////////
			for( int i = 0 ; i < NB_EDGES ; i++ ) {
...
//	glOrtho( -1.0 * ratio , 1.0 * ratio , -1.0 , 1.0 , -10.0, 10.00 );
	...
	//printf( "%.5f " , mu );
		...
		osc::OutboundPacketStream p( buffer, OUTPUT_BUFFER_SIZE );
		p //<< osc::BeginBundleImmediate
		<< osc::BeginMessage( "/position_xyz" ) <<(int) ind <<(float)dataFloatTrace[4 * ind + 0] << (float)dataFloatTrace[4 * ind + 1] << (float)dataFloatTrace[4 * ind + 2] << osc::EndMessage;
		//<< osc::EndBundle;
		transmitSocket.Send( p.Data(), p.Size() );
	}
	...
	// if( false ) 
	{
		...
		// pas d'�clairement, c'est fait par les shaders
