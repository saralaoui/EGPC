...
Copyright (C) 2006 University Paris 11 
...
//Paramètres à gérer par le biais d'un controle gestuel

//raideur supplémentaire
float Kplus = 0;
//tableaux de raideurs supplémentaires
float RAIDEURTABLEMODIF[50];
int indice_raideur_max = 50;
//longueure initiale supplémentaire
float Linit = 0;
//amortissement total supplémentaire
float Amort = 0;
//constante de viscosité
float mu = 0;
//masse uniforme supplémentaire
float MassePlus = 0;
float scale =1;
...
//topologie choisie en fonction des cas
int topologie=1;
...
GLuint g_frameBufferraideur;
GLuint g_frameBufferraideur2;
...
// programmes GC
// pass 0: raideur supplémentaire
CGprogram cgQuadVertexProgram;					// TD5-quad-VP.cg
CGprogram cgRaideurPlusFragmentProgram;         // TD5-raideurPlus-FS.cg
// pass 0Bis: raideur additionnée
//même VP que la pass 0
CGprogram cgRaideurPlusCopieFragmentProgram;         // TD5-raideurPlusCopie-FS.cg
// pass 1: forces
//même VP que la passe 0
...
// paramÈtres Cg pour le programme de calcul des forces
// pass 0 : raideur supplémentaire
CGparameter texture_FragmentProgram;                       // texture
//CGparameter edge_Index_K_L0_ForceFragmentProgram;          // edge_Index_K_L0
CGparameter edge_Index_K_L0_raideurPlusFragmentProgram;		//texture des raideurs supplémentaires
CGparameter tablemodifraideur_plusFragmentProgram;		  //tableau de raideurs supplémentaires
CGparameter indice_raideur_maxFragmentProgram;			 //indice dans le tableau des raideurs max
// pass 0 : raideur additionnée
//CGparameter edge_Index_K_L0_raideurPlusFragmentProgram		//texture des raideurs supplémentaires
// pass 1: forces
//CGparameter texture_FragmentProgram;                       // texture
CGparameter edge_Index_K_L0_ForceFragmentProgram;          // edge_Index_K_L0
...
// pass 4: positions
// paramÈtres Cg pour le programme de calcul des positions
...
CGparameter muFragmentShaderParam;							// constante de viscosité
CGparameter MassePlusFragmentShaderParam;					// masse supplementaire  pour les masses
// pass 5: dessin
// paramÈtres Cg pour le programme de dessin
...
int main(int argc, char **argv)  { 
	...
	//paramètres du main, la topologie, le nb de masses et de ressorts



  // initialisation  des paramËtres de GLUT en fonction
  // des arguments sur la ligne de commande
  glutInit(&argc, argv);
  glutInitDisplayMode( GLUT_RGBA | GLUT_DOUBLE | GLUT_DEPTH );

  // dÈfinition et crÈation de la fenÍtre graphique
  glutInitWindowSize(width, height);
  glutInitWindowPosition(0, 0);
  glutCreateWindow("TD5");

  // initialisation de OpenGL de Cg et de la scËne
  initGL();  
  initCg();  
  init_scene();
	...
	int thread_id;
	char *nothing;
	// crée un thread qui execute en parallèle la fonction ConnectListener ci-dessous avec des attributs par défaut
	thread_id = pthread_create( &listener_thread, NULL, ConnectListener , (void*) nothing);
	
	
	
  // choix des procÈdures de callback pour 
  // le tracÈ graphique
  glutDisplayFunc(&window_display);
  // le redimensionnement de la fenÍtre
  glutReshapeFunc(&window_reshape);
  // la gestion des ÈvÈnements clavier
  glutKeyboardFunc(&window_key);
  // la gestion des clicks souris
  glutMouseFunc(&window_mouseFunc);
  // la gestion des dÈplacement souris
  glutMotionFunc(&window_motionFunc);
  // fonction appelÈe rÈguliËrement entre deux gestions d¥ÈvÈnements
  glutTimerFunc(0,&window_timer,FrameNo);

  // la boucle prinicipale de gestion des ÈvÈnements utilisateur
  glutMainLoop();  
...
// fonction OSC 
void *ConnectListener(void *message) {
	//défini la méthode de traitement des données 
	SarahPacketListener listener;
	//défini la communication UDP et permet de recevoir les pacquets
...
GLvoid initGL()  {
  ...
  // DÈfinit un modËle d¥ombrage
  glShadeModel(GL_SMOOTH);

  // Z Buffer pour la suppression des parties cachÈes
...
GLvoid initCg()  {
  ...
  // crÈation d'un contexte pour les programmes Cg
  ...
  cgGLSetOptimalOptions(cgFragmentProfile);
	...
	// compile le programme de vertex ‡ partir d'un fichier
    ...
    = cgCreateProgramFromFile( cgContext,        // le context Cg courant
							  ...
							  "main",           // le programme d'entrÈe
							  NULL );           // autres paramËtres
	// + vÈrification
	if( cgQuadVertexProgram == NULL ) {
		// Cg nous renvoie un pointeur vers une erreur
		CGerror Error = cgGetError();
		
		// dont on affiche la chaine - gÈnÈralement peu informative...
		printf( "Chargement incorrect du programme de vertex de calcul forces %s (%s)\n" ,
			   filename , cgGetErrorString(Error) ); 
		exit( 0 );
	}
	
	// compile le programme de fragment ‡ partir d'un fichier
	strcpy( filename , "src/TD5-raideurPlus-FS.cg" );
	printf( "Chargement du programme de fragment %s\n" , filename );
	cgRaideurPlusFragmentProgram 
    = cgCreateProgramFromFile( cgContext,        // le context Cg courant
							  CG_SOURCE,        // programme source
							  // vs. CG_OBJECT pour un
							  // programme objet
							  ...
							  NULL );           // autres paramËtres
	// + vÈrification
	if( cgRaideurPlusFragmentProgram == NULL ) {
		// Cg nous renvoie un pointeur vers une erreur
		CGerror Error = cgGetError();
	...
	/////////////////////////////////////////////////////
	// pass 0Bis: recopie après addition des raideur plus
	// compilation des programmmes de vertex et de fragment de calcul des forces
	
		
	// compile le programme de fragment ‡ partir d'un fichier
	strcpy( filename , "src/TD5-raideurPlusCopie-FS.cg" );
	printf( "Chargement du programme de fragment %s\n" , filename );
	cgRaideurPlusCopieFragmentProgram 
    = cgCreateProgramFromFile( cgContext,        // le context Cg courant
							  CG_SOURCE,        // programme source
							  // vs. CG_OBJECT pour un
							  // programme objet
							  ...
							  NULL );           // autres paramËtres
	// + vÈrification
	if( cgRaideurPlusCopieFragmentProgram == NULL ) {
		// Cg nous renvoie un pointeur vers une erreur
		CGerror Error = cgGetError();
  ...
  // mÍme programme de vertex que le calcul des forces
  ...
  if( cgDessinFragmentProgram == NULL ) {
	...
	cgGLLoadProgram(cgRaideurPlusFragmentProgram);
	
	//  et liens avec ses paramËtres
	edge_Index_K_L0_ForceFragmentProgram 
    = cgGetNamedParameter(cgRaideurPlusFragmentProgram, "edge_Index_K_L0");
	tablemodifraideur_plusFragmentProgram 
    = cgGetNamedParameter(cgRaideurPlusFragmentProgram, "tablemodif");
	indice_raideur_maxFragmentProgram 
    = cgGetNamedParameter(cgRaideurPlusFragmentProgram, "indice_raideur_max");
	
	if (!edge_Index_K_L0_ForceFragmentProgram
		||!tablemodifraideur_plusFragmentProgram
		||!indice_raideur_maxFragmentProgram) {
		printf("Erreur dans le chargement des paramËtres du programme de fragment de calcul des raideurs\n");
		exit(0);
	}

	/////////////////////////////////////////////////
	// pass 0Bis :recopie après addition de raideur Plus
	// chargement des programmmes de vertex et de fragment de calcul des forces
	...
	// chargement du programme de fragment de calcul des forces
	cgGLLoadProgram(cgRaideurPlusCopieFragmentProgram);
	
	//  et liens avec ses paramËtres
	edge_Index_K_L0_raideurPlusFragmentProgram
    = cgGetNamedParameter(cgRaideurPlusCopieFragmentProgram, "raideur_plus");
	
	
	if (!edge_Index_K_L0_raideurPlusFragmentProgram) {
		printf("Erreur dans le chargement des paramËtres du programme de fragment de calcul de la copie des raideurs\n");
		exit(0);
	}
  ...
  if (!edge_Index_K_L0_ForceFragmentProgram 
	  ...
	  || !LinitFragmentShaderParam
	  || !AmortFragmentShaderParam) {
    printf("Erreur dans le chargement des paramËtres du programme de fragment de calcul des forces\n");
    exit(0);
  }
  ...
  if (!edge_Index_K_L0_LeftForcesVertexShaderParam ) {
    printf("Erreur dans le chargement des paramËtres du programme de vertex de forces gauche\n");
    exit(0);
  }
  ...
  if (!edge_Index_K_L0_RightForcesVertexShaderParam  ) {
    printf("Erreur dans le chargement des paramËtres du programme de vertex de forces droite\n");
    exit(0);
  }
  ...
  if (!edge_Force_LeftFragmentShaderParam ) {
    printf("Erreur dans le chargement des paramËtres du programme de fragment de forces gauche\n");
    exit(0);
  }
  ...
  if (!edge_Force_RightFragmentShaderParam ) {
    printf("Erreur dans le chargement des paramËtres du programme de fragment de forces droite\n");
    exit(0);
  }
  /////////////////////////////////////////////////
  // pass 4: positions
  ...
  // mÍme programme de vertex que les forces
  ...
  if (   !mass_Forces_PositionsFragmentShaderParam 
         || !mass_PrevPos_PositionsFragmentShaderParam 
         || !mass_CurrPos_PositionsFragmentShaderParam
	     || !scale_PositionsFragmentShaderParam
	     || !MassePlusFragmentShaderParam
	     || !muFragmentShaderParam) {
    printf("Erreur dans le chargement des paramËtres du programme de fragment de positions\n");
    exit(0);
  }
  ...
  if (!largeurParticlesVertexShaderParam 
       || !profondeurParticlesVertexShaderParam 
       || !mass_Positions_DessinVertexShaderParam ) {
    printf("Erreur dans le chargement des paramËtres du programme de vertex de dessin\n");
    exit(0);
  }
  ...
  if ( !decal_DessinFragmentShaderParam ) {
    printf("Erreur dans le chargement des paramËtres du programme de fragment de dessin\n");
    exit(0);
  }
}
...
///////////////////////////////////////
// CHARGEMENT DES DONN…ES DANS LA GPU: display lists et textures
...
void init_scene() {
  ...
  glBindFramebufferEXT( GL_FRAMEBUFFER_EXT, 0 );
	...
	////////////// INIT DU FBO de raideur supplémentaires
	//glGenFramebuffersEXT( 1, &g_frameBufferraideur );
	//glGenFramebuffersEXT( 1, &g_frameBufferraideur2 );
	//glGenFramebuffersEXT( ... );
	//glGenTextures(1, &g_Texture[RAIDEUR_PLUS_2_TEXTURE] );
	//glBindTexture(GL_TEXTURE_RECTANGLE_ARB, g_Texture[RAIDEUR_PLUS_2_TEXTURE]);
	//glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA_FLOAT32_ATI ,
	//			 sizeX, sizeY, 0, GL_RGBA, 
	//			 GL_FLOAT, (const void *)dataFloat );	contenu dans le CreateTexture();
	
	////////////// Utilisation du FBO de raideur supplémentaire
	// BRANCHER LE FBO
	//glBindFramebufferEXT(GL_FRAMEBUFFER_EXT , g_frameBufferraideur);
	//CreateTexture( RAIDEUR_PLUS_TEXTURE , 512 , 512 );
	// DEBRANCHER LE FBO
	//glBindFramebufferEXT(	GL_FRAMEBUFFER_EXT , 0);
...
// fonction de call-back appelÈe rÈguliËrement
...
// fonction de call-back de glut pour l¥affichage dans la fenÍtre

GLvoid window_display() {
  render_scene();

  // trace la scËne grapnique qui vient juste d'Ítre dÈfinie
  glFlush();
}

// fonction de call-back de glut pour le redimensionnement de la fenÍtre
...
// fonction de call-back de glut pour la gestion des ÈvÈnements clavier
...
// fonction de call-back de glut de gestion des dÈplacement de la souris
...
// pour une texture RECT (coordonnÈes de texture entre 
// 0 et width) dans le FS
// et via la normale pour une texture 2D (coordonnÈes 
// de texture entre 0 et 1) dans le VP
void MakeQuadRECT2D( void ) {
  glNewList(My_QuadRECT2D, GL_COMPILE);

    glBegin(GL_POLYGON);
	//texcoord0 decal2Dcoord
      glNormal3f(0.0 , 0.0 , 0.0);
	//texcoord1 decalRECTcoord
...
void MakeParticles( void ) {
  ...
  glNewList(My_Particles, GL_COMPILE);
    for( int ind = 0 ; ind < NB_MASSES ; ind++ ) {
      glBegin(GL_POLYGON);
	glTexCoord2f(0.0 , 0.0);
	glVertex3f(  (float)(ind) , 0.0 , -0.01 );
 
	glTexCoord2f(128.0 , 0.0);
	glVertex3f(  (float)(ind) , 1.0 , -0.01 ); 

	glTexCoord2f(128.0 , 128.0);
	glVertex3f(  (float)((ind+1)%NB_MASSES) , 1.0 , -0.01 ); 

	glTexCoord2f(0.0 , 128.0);
	glVertex3f(  (float)((ind+1)%NB_MASSES) , 0.0 , -0.01 ); 
      glEnd();
    }
    for( int ind = 0 ; ind < NB_MASSES ; ind++ ) {
      glBegin(GL_POLYGON);
	...
	glTexCoord2f(128.0 , 0.0);
	glVertex3f(  (float)(ind) , 1.0 , 0.0 ); 

	glTexCoord2f(128.0 , 128.0);
	glVertex3f(  (float)(ind) , 2.0 , 0.0 ); 

	glTexCoord2f(0.0 , 128.0);
	glVertex3f(  (float)(ind) , 3.0 , 0.0 ); 
      glEnd();
    }
  glEndList();
}
...
void MakePoints( void ) {
  // 1 point correspond ‡ une masse au bout d'une arÍte
  // il est rendu ‡ la position dans la texture de force
  // (donc aux coordonnÈes de texture d'une masse dans 
  // une texture de donnÈes de masses)
  glNewList(My_Points, GL_COMPILE);
    ...
    // les masses sont positionnÈes au bon endroit du frame buffer
...
void CreateTexture( int textureId , int sizeX , int sizeY ) {
  ...
  case POSITION_1_TEXTURE:
    {
      /////////////////////////////////////////////////////////////////////
      // texture qui sert ‡ stocker les informations sur les aretes
      // (indices des masses, k, l0
      // utilisÈe en shader de vertex et de fragment
	...
	printf( "Texture allocation error!" ); throw 425;
	  ...
	  float alpha = M_PI/4;
		...
		for( int ind = 0 ; ind < NB_MASSES ; ind++ ) {
			...
			dataFloat[ ind * 4 ]     = R * cos( alpha );
			dataFloat[ ind * 4 + 1 ] = R * sin( alpha );
			dataFloat[ ind * 4 + 2 ] = 0.0;
			dataFloat[ ind * 4 + 3 ] = 1.0;
		}		
	...
	// cibles d'Ècriture pour le FBO: la position prÈcÈdente, 
	// la position courante et la position antÈrieure. 
	// On a besoin de ces trois valeurs pour
	// calculer le dÈplacement dans le schÈma d'intÈgration de Verlet
	// VTF (vertex texture fetch) donc format GL_RGBA_FLOAT32_ATI
	// attachÈe ‡ un FBO 
	// initialisÈe par les valeurs stockÈes dans dataFloat
	glTexImage2D( GL_TEXTURE_2D , 0, GL_RGBA_FLOAT32_ATI , 
		      sizeX, sizeY, 
		      // dataFloat est NULL pour POSITION_3_TEXTURE
		      // qui n'a pas besoin d'etre initialisÈe
		      0, GL_RGBA, GL_FLOAT, (const void *)dataFloat );
  ...
  case FORCE_TEXTURE:
    {
      ...
      /////////////////////////////////////////////////////////////////////
      // texture qui sert ‡ stocker les forces avant de les cumuler
      // attachÈe ‡ un FBO et sans initialisation nÈcessaire
  ...
  case FORCE_MASS_TEXTURE:
    {
      ...
      /////////////////////////////////////////////////////////////////////
      // texture qui sert ‡ cumuler les forces 
      // comme le cumul se fait par blending accumulatif,
      // la prÈcision doit Ítre 16f (le blending sur les textures 
      // 32f clampe les valeurs ‡ 0..1)
      // attachÈe ‡ un FBO et sans initialisation nÈcessaire
  ...
  case EDGE_TEXTURE:
    {
      ...
      glEnable( GL_TEXTURE_2D );
      // Lie la texture avec une cible carrÈe
	...
	dataFloat[ i * 4 ]     = (GLfloat)(i % NB_MASSES);
	dataFloat[ i * 4 + 1 ] = (GLfloat)((i + 1) % NB_MASSES);
	dataFloat[ i * 4 + 2 ] = (((float)rand() / (float)RAND_MAX) + 1) * 0.3;
	dataFloat[ i * 4 + 3 ] = (((float)rand() / (float)RAND_MAX) + 1) 
	                         * 2 * M_PI / (float)NB_MASSES;
      }
      for( int i = NB_MASSES ; i < sizeX * sizeY ; i++ ) {
	dataFloat[ i * 4 ]     = 0;
	dataFloat[ i * 4 + 1 ] = 0;
    ...
    break;
	  case RAIDEUR_PLUS_TEXTURE:
	  {
		  ...
		  /////////////////////////////////////////////////////////////////////
		  // texture qui sert à stocker les raideurs supplémentaires	
...
//retour d'info en OSC vers MaxMSP
	...
	<< osc::BeginMessage( "/mu" ) << (float)0.5 << osc::EndMessage;
	//<< osc::EndBundle;
  ...
  // et celle des positions antÈrieures
  ...
  glGetIntegerv(GL_DRAW_BUFFER, &glut_draw_buffer);
	...
	/////////////////////////////////////////////////////////
	// pass 0-: forces
	// CALCUL DES raideurs supplémentaires
	/////////////////////////////////////////////////////////
  ...
  cgGLEnableProfile(cgFragmentProfile);
	...
	// raideur supplémentaire sur les arêtes
	cgGLBindProgram(cgQuadVertexProgram);
	cgGLBindProgram(cgRaideurPlusFragmentProgram);
	
	// lie les paramËtres uniformes au programme de vertex
    ...
    /////////////////////////////////////////////////////////
    // texture des arËtes et des masses
    cgGLSetTextureParameter( edge_Index_K_L0_ForceFragmentProgram, 
							g_Texture[EDGE_TEXTURE] );
	//le tableau de raideur supplémentaires 
	cgGLSetParameterArray2f( tablemodifraideur_plusFragmentProgram, 0, 50,
							RAIDEURTABLEMODIF );
	// fait le lien entre les paramètres de contrôle uniformément pris par CG 
	cgGLSetParameter1f(indice_raideur_maxFragmentProgram, indice_raideur_max);
	
	
	
	// BRANCHER LE 1ER FBO -> FBO en écriture
	//T2=

	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT , g_frameBufferraideur);
	glFramebufferTexture2DEXT( GL_FRAMEBUFFER_EXT , GL_COLOR_ATTACHMENT0_EXT , 
							  GL_TEXTURE_RECTANGLE_ARB , 
							  g_Texture[RAIDEUR_PLUS_TEXTURE] , 0 );
	
	//brancher la texture en lecture
	//=T1
	cgGLEnableTextureParameter( edge_Index_K_L0_ForceFragmentProgram);
	
	// raideurplus = edge + tableau de raideur supplémentaire
	//T2=T1+tableau de raideur plus
	//activer le shader
	glCallList( My_QuadRECT2D );
	
	// DEBRANCHER LE 1er FBO 
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT , 0);
	//débrancher la texture
	...
	/////////////////////////////////////////////////////////
	// pass 0-bis: forces
	// RECOPIE DES raideurs supplémentaires
	/////////////////////////////////////////////////////////
	
	
	// raideur supplémentaire sur les arêtes
	cgGLBindProgram(cgQuadVertexProgram);
	cgGLBindProgram(cgRaideurPlusCopieFragmentProgram);
	...
	// BRANCHER LE 2ER FBO -> FBO en lecture
	//T1=
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT , g_frameBufferraideur2);
	glFramebufferTexture2DEXT( GL_FRAMEBUFFER_EXT , GL_COLOR_ATTACHMENT0_EXT , 
							  GL_TEXTURE_RECTANGLE_ARB , 
							  g_Texture[EDGE_TEXTURE] , 0 );
	//brancher la texture en lecture
	//=T2
	cgGLEnableTextureParameter( edge_Index_K_L0_raideurPlusFragmentProgram);
	
	indice_raideur_max =0;
	
	glCallList( My_QuadRECT2D );
	
	// DEBRANCHER LE 2EME FBO -> FBO en écriture
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT , 0);
	//débrancher la texture
	cgGLDisableTextureParameter(edge_Index_K_L0_raideurPlusFragmentProgram);	
	...
	// toutes les transformations suivantes s¥appliquent au modËle de vue 
  ...
  glPushMatrix();
	...
	//paramètre de masse supplémentaire et de viscosité
	cgGLSetParameter1f(MassePlusFragmentShaderParam, MassePlus);
	cgGLSetParameter1f(scale_PositionsFragmentShaderParam, scale);
...
//   /////////////////////////////////////////////////////////
  ...
  // pas d'Èclairement, c'est fait par les shaders
  ...
  glPushMatrix();
    ...
    cgGLSetParameter1f( largeurParticlesVertexShaderParam , 0.2 ); 
